...
static int kvm_mmu_notifier_invalidate_range_start(
  ...
  ret = kvm_arch_mmu_notifier_invalidate_range(kvm, range->start, range->end,
                                               range->blockable);
...
static int kvm_create_vm_debugfs(struct kvm *kvm, int fd) {
  ...
  kvm->debugfs_stat_data =
      kcalloc(kvm_debugfs_num_entries, sizeof(*kvm->debugfs_stat_data),
              GFP_KERNEL_ACCOUNT);
  if (!kvm->debugfs_stat_data)
    return -ENOMEM;
  ...
  for (p = debugfs_entries; p->name; p++) {
    ...
    debugfs_create_file(p->name, 0644, kvm->debugfs_dentry, stat_data,
...
static void kvm_destroy_vm(struct kvm *kvm) {
  ...
  if (main_vm == kvm) {
    printk("main vm is destoryed, reseting my status variables");
    spin_lock(&kaslr_fault_counter_lock);
    kaslr_fault_counter = 0;
    spin_unlock(&kaslr_fault_counter_lock);

    spin_lock(&kaslr_attack_lock);
    kaslr_attacked = false;
    spin_unlock(&kaslr_attack_lock);
  }
}

void kvm_get_kvm(struct kvm *kvm) { refcount_inc(&kvm->users_count); }
...
static int hva_to_pfn_remapped(struct vm_area_struct *vma, unsigned long addr,
  ...
  if (r) {
    ...
    r = fixup_user_fault(current, current->mm, addr,
                         (write_fault ? FAULT_FLAG_WRITE : 0), &unlocked);
    if (unlocked)
      return -EAGAIN;
    if (r)
      return r;
...
static int __kvm_read_guest_page(struct kvm_memory_slot *slot, gfn_t gfn,
  ...
  if (kvm_is_error_hva(addr)) {
    printk(KERN_CRIT "gfn_to_hva_memslot_prot failed\n");
    return -EFAULT;
  }
  r = __copy_from_user(data, (void __user *)addr + offset, len);
  if (r) {
    printk(KERN_CRIT "__copy_from_user failed\n");
    return -EFAULT;
  }
  return 0;
}
...
static long get_user_pages_remote_unlocked(struct mm_struct *mm,
  ...
  ret = get_user_pages_remote(tsk, mm, start, nr_pages, gup_flags, pages, vmas,
                              &locked);
  if (locked)
    up_read(&mm->mmap_sem);
  return ret;
...
static int kvm_dev_ioctl_create_vm(unsigned long type) {
  ...
  fd_install(r, file);
  if (main_vm != NULL) {
    printk(KERN_WARNING "Luca: Overwriting main_kvm\n");
  }
  main_vm = kvm;

  // reset fault counter for choose_random_location function
  spin_lock(&kaslr_fault_counter_lock);
  kaslr_fault_counter = 0;
  spin_unlock(&kaslr_fault_counter_lock);
...
put_kvm:
  kvm_put_kvm(kvm); // decreases the refcount and tests if this object is used
                    // anymore if not it's destroyed
  return r;
}
...
static long kvm_dev_ioctl(struct file *filp, unsigned int ioctl,
                          unsigned long arg) {
  long r = -EINVAL;
  dump_page_param_t dump_page_param;
  write_page_param_t write_page_param;
  ...
  char line[32];
  char *p; // a pointer to the line for using strsep() function since it takes
           // (char**)
  ...
  void *buffer;
  __u8 *buffer_bytewise;
  int res, i;
  const int block_size = 128;
  decrypt_at_host = false;
  ...
  case KVM_TRACKING_ENABLE:
    ...
    if (severed_action == SEVERED_NONE) {
      if (copy_from_user(severed_strpid_param.str_pid, argp,
                         sizeof(severed_strpid_param)))
        return -EFAULT;

      n = strlen(severed_strpid_param.str_pid);
      printk(KERN_DEBUG
             "SEVered: I am here in kmv_main.c case KVM_TRACKING_ENABLE size "
             "str_for_pid %d sizeof(severed_strpid_param) %ld\n",
             n, sizeof(severed_strpid_param));
      ...
      if (ret) {
        ...
        printk(KERN_DEBUG "SEVered: Started tracking pid %d\n",
               severed_pid_from_shell);
  ...
  case KVM_MAPPING_CHANGE:
    ...
    if (severed_action == SEVERED_NONE) {
      ...
      c = strsep(&p, ",");
      if (!c) {
        printk("SEVered: something wrong with the input for the case "
               "KVM_MAPPING_CHANGE, example: 4537,000023a5,0000f365  where "
               "pid,gfn1,gfn2\n");
        r = 0;
        spin_unlock(&severed_action_lock);
        break;
    ...
    spin_unlock(&severed_action_lock);
    break;
#define KVM_WRITE_PAGE_DEBUG
  case KVM_WRITE_PAGE:
    argp = (void __user *)arg; // not sure what this is for
    printk(KERN_NOTICE "Luca: KVM_WRITE_PAGE: got called on VM with pid %d\n",
           main_vm->userspace_pid);
    // copy parameters from user space
    if (copy_from_user(&write_page_param, argp, sizeof(write_page_param_t))) {
      printk(KERN_CRIT
             "Luca: KVM_WRITE_PAGE: error copying arguments, exiting\n");
      return -EFAULT;
    }
#ifdef KVM_WRITE_PAGE_DEBUG
    printk(KERN_NOTICE "Luca: KVM_WRITE_PAGE: copied arguments\n");
#endif

    new_data_buffer = kmalloc(write_page_param.length, GFP_KERNEL);
    if (new_data_buffer == NULL) {
      printk(KERN_CRIT "Luca: KVM_WRITE_PAGE: failed to allocate "
                       "new_data_buffer, exiting\n");
      return r;
    }

    res = copy_from_user(new_data_buffer,
                         (void *)write_page_param.user_space_buffer,
                         write_page_param.length);
    if (res != 0) {
      printk(KERN_CRIT
             "Luca: KVM_WRITE_PAGE: failed to copy from userpace, exiting\n");
      return r;
    }

#ifdef KVM_WRITE_PAGE_DEBUG
    printk(KERN_NOTICE
           "Luca: KVM_WRITE_PAGE: Copied new data from user space\n");
#endif

    buffer_bytewise = (__u8 *)new_data_buffer;
    printk(KERN_NOTICE
           "Luca: KVM_DUMP_WRITE: First 32 bytes from user space\n");
    for (i = 0; i < 32; i++) {
      if ((i % block_size == 0)) {
        printk(KERN_NOTICE "Line %d :", i / block_size);
      }
      printk(KERN_CONT " %02x ", buffer_bytewise[i]);
      if (i != 0 && (i % block_size == 0)) {
        printk(KERN_CONT "/n");
      }
    }

    res = write_physical(main_vm, write_page_param.gpa, write_page_param.length,
                         new_data_buffer, false);
    if (res != 0) {
      printk(KERN_CRIT
             "Luca: KVMI_WRITE_PAGE: kvmi_cmd_write_physical failed with %d \n",
             res);
      kfree(new_data_buffer);
      return res;
    }

#ifdef KVM_WRITE_PAGE_DEBUG
    printk(KERN_NOTICE "Luca: KVM_WRITE_PAGE: Wrote data into page\n");
#endif
    ...
    r = 0;
    break;
  case KVM_DUMP_PAGE_HOSTDEC:
    decrypt_at_host = true; // falltrough is desired behaviour
    // see https://github.com/batmac/wbinvd/blob/master/wbinvd.c
    // currently i do this in the dump script for performance reasons
    // wbinvd_on_all_cpus(); // according to amd prog man 15.34.7 necessary

  case KVM_DUMP_PAGE:
    //#define KVM_DUMP_PAGE_DEBUG
    argp = (void __user *)arg; // not sure what this is for
#ifdef KVM_DUMP_PAGE_DEBUG
    printk(KERN_NOTICE "Luca: KVM_DUMP_PAGE: got called on VM with pid %d\n",
           main_vm->userspace_pid);
#endif
    // copy parameters from user space
    if (copy_from_user(&dump_page_param, argp, sizeof(dump_page_param_t))) {
      printk(KERN_CRIT
             "Luca: KVM_DUMP_PAGE: error copying arguments, exiting\n");
      return -EFAULT;
    }

#ifdef KVM_DUMP_PAGE_DEBUG
    printk(KERN_NOTICE "Luca: KVM_DUMP_PAGE: copied arguments\n");
    printk(KERN_NOTICE "Luca: KVM_DUMP_PAGE: going to read gpa %016llx\n",
           dump_page_param.gpa);
#endif
    ...
    if (buffer == NULL) {
      printk(KERN_CRIT
             "Luca: KVM_DUMP_PAGE: Failed to allocate buffer,exiting\n");
      return r;
    }

    res = read_physical(main_vm, dump_page_param.gpa, buffer,
                        dump_page_param.length, decrypt_at_host);
    if (res != 0) {
      printk(KERN_CRIT "Luca: kvm_read_guest failed with %d , exiting\n", res);
      return res;
    }

#ifdef KVM_DUMP_PAGE_DEBUG
    printk(KERN_NOTICE
           "Luca: KVM_DUMP_PAGE: Copying page to user space address %016llx\n",
           dump_page_param.user_space_buffer);
#endif
    res = copy_to_user((void *)dump_page_param.user_space_buffer, buffer,
                       dump_page_param.length);
    if (res != 0) {
      printk(KERN_CRIT
             "Luca: KVM_DUMP_PAGE: copy_to_user failed with : %d, exiting \n",
             res);
      kfree(buffer);
      return res;
    }
    ...
    r = 0;
    break;
  case KVM_LOAD_KNOWN_PLAINTEXT:
#define KVM_LOAD_KNOWN_PLAINTEXT_DEBUG
    printk(KERN_NOTICE "Luca: KVM_LOAD_KNOWN_PLAINTEXT: got called\n");

    argp = (void __user *)arg; // not sure what this is for
    // copy parameters from user space
    if (copy_from_user(&load_plaintext_param, argp,
                       sizeof(load_plaintext_param_t))) {
      printk(KERN_CRIT "Luca: KVM_LOAD_KNOWN_PLAINTEXT: error copying "
                       "arguments, exiting\n");
      return -EFAULT;
    }
#ifdef KVM_LOAD_KNOWN_PLAINTEXT_DEBUG
    printk(KERN_NOTICE "Luca: KVM_LOAD_KNOWN_PLAINTEXT: copied arguments\n");
#endif

    new_data_buffer = vmalloc(load_plaintext_param.length);
    if (new_data_buffer == NULL) {
      printk(KERN_CRIT "Luca: KVM_WRITE_PAGE: failed to allocate "
                       "new_data_buffer, exiting\n");
      return r;
...
#ifdef KVM_LOAD_KNOWN_PLAINTEXT_DEBUG
    printk("Going to copy %llu bytes from user space addr %llx\n",
           load_plaintext_param.length, load_plaintext_param.user_space_buffer);
#endif
    res = copy_from_user(new_data_buffer,
                         (void *)load_plaintext_param.user_space_buffer,
                         load_plaintext_param.length);
    if (res != 0) {
      printk(KERN_CRIT
             "Luca: KVM_WRITE_PAGE: failed to copy from userpace, exiting\n");
      return r;
    ...
    // add to db
    if (load_plaintext_param.length % CIPHER_BLOCK != 0) {
      printk("in %s line %d: warning load_plaintext_param.length is not a "
             "multiple of CIPHER_BLOCK\n",
             __FILE__, __LINE__);
    }
    if (db != NULL) {
      printk("db was not null, free db\n");
      free_db();
    }
    printk("allocating memory for db...");
    // TODO: Function is called with vm memory inbytes. Include this as an
    // parameter
    // TODO *2 is temporary for debugging
    err = alloc_db(VM_MEMORY_IN_MB * 1024 * 1024 * 2);
    // err = alloc_db(2 * 4096);
    if (err != 0) {
      printk("allocation failed with %d\n", err);
      vfree(new_data_buffer);
      return r;
    }
    printk("done\n");

    for (offset = 0; offset < load_plaintext_param.length;
         offset += CIPHER_BLOCK) {
      // this assumes that the plaintext is continous in gpa memory
      uint64_t gpa = load_plaintext_param.gpa + offset;
//#ifdef KVM_LOAD_KNOWN_PLAINTEXT
      if ((gpa & 0xFFF) == 0) {
        printk("KVM_LOAD_KNOWN_PLAINTEXT: processing data for gpa"
      "0x%llx\n", gpa);
      }
//#endif
      if (!db_add_incomplete(gpa, ((uint8_t *)new_data_buffer) + offset)) {
        printk(
            "in %s line %d: failed to add incomplete entry for gpa=%016llx\n",
            __FILE__, __LINE__, gpa);
      }
    }
    printk("KMV_LOAD_KNOWN_PLAINTEXT: processed data, freeing buffer and "
           "exiting\n");
    vfree(new_data_buffer);
...
int kvm_io_bus_register_dev(struct kvm *kvm, enum kvm_bus bus_idx, gpa_t addr,
  ...
  new_bus =
      kmalloc(struct_size(bus, range, bus->dev_count + 1), GFP_KERNEL_ACCOUNT);
  if (!new_bus)
    return -ENOMEM;
...
static int vm_stat_get_per_vm_open(struct inode *inode, struct file *file) {
  ...
  return kvm_debugfs_open(inode, file, vm_stat_get_per_vm, vm_stat_clear_per_vm,
                          "%llu\n");
}
...
static int vcpu_stat_get_per_vm(void *data, u64 *val) {
  ...
  *val = 0;
  ...
  kvm_for_each_vcpu(i, vcpu, stat_data->kvm) *val +=
      *(u64 *)((void *)vcpu + stat_data->offset);
...
static void kvm_init_debug(void) {
  ...
  for (p = debugfs_entries; p->name; ++p, kvm_debugfs_num_entries++) {
    debugfs_create_file(p->name, 0644, kvm_debugfs_dir, (void *)(long)p->offset,
                        stat_fops[p->kind]);
  }
}
