...
typedef struct {
  __u64 gpa;                   // we inject here
  __u8 *injection_code_buffer; // code that gets injected (without tweaks)
  __u64 length;                // length of the code
  __u8 insert_at_back; // if 1, inject at back of 16 byte block, else at front
  __u32 target_value;  // the register should have this value after the gadget
  __u32 prev_prev_eax; // values of the eax register after cpuid calls. If cpuid
                       // is called with eax=0x0 it wrties 0xd to eax, so the
                       // value toggles
  __u32 prev_eax;
  __u32 type; // 1 = counter_gadget, 2 = stack detect gadget
} inject_param_t;
