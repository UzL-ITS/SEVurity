...
static inline bool is_error_page(struct page *page) { return IS_ERR(page); }
...
#define KVM_ARCH_REQ_FLAGS(nr, flags)                                          \
  ({                                                                           \
    BUILD_BUG_ON((unsigned)(nr) >=                                             \
                 (FIELD_SIZEOF(struct kvm_vcpu, requests) * 8) -               \
...
static inline int kvm_arch_vcpu_memslots_id(struct kvm_vcpu *vcpu) { return 0; }
#endif
...
#define kvm_debug_ratelimited(fmt, ...)                                        \
  pr_debug_ratelimited("kvm [%i]: " fmt, task_pid_nr(current), ##__VA_ARGS__)
#define kvm_pr_unimpl(fmt, ...)                                                \
  pr_err_ratelimited("kvm [%i]: " fmt, task_tgid_nr(current), ##__VA_ARGS__)
...
static inline struct kvm_vcpu *kvm_get_vcpu_by_id(struct kvm *kvm, int id) {
  ...
  if (vcpu && vcpu->vcpu_id == id)
    return vcpu;
  kvm_for_each_vcpu(i, vcpu, kvm) if (vcpu->vcpu_id == id) return vcpu;
  return NULL;
}
...
static inline int kvm_vcpu_get_idx(struct kvm_vcpu *vcpu) {
  ...
  kvm_for_each_vcpu(idx, tmp, vcpu->kvm) if (tmp == vcpu) return idx;
  BUG();
}
...
#else
static inline bool kvm_arch_intc_initialized(struct kvm *kvm) { return true; }
#endif
...
struct kvm_device_ops {
  ...
  long (*ioctl)(struct kvm_device *dev, unsigned int ioctl, unsigned long arg);
};
...
#else
static inline bool vcpu_valid_wakeup(struct kvm_vcpu *vcpu) { return true; }
